...
public class ManagerService {
	...
	public Manager saveManager(
		...
		if (!ObjectUtils.nullSafeEquals(user.getId(), todo.getUser().getId())) {
			throw new InvalidRequestException("담당자를 등록하려고 하는 유저가 일정을 만든 유저가 아닙니다.");
		}
		...
		return managerRepository.save(new Manager(managerUser, todo));
	}

	public List<Manager> getManagers(long todoId) {
		return managerRepository.findAllByTodoId(todoId);
	}
